...
import (
	...
	pb "github.com/FIzore/PolarisCache/pb"
...
func NewClient(addr string, svcName string, etcdCli *clientv3.Client) (*Client, error) {
	...
	grpcClient := pb.NewPolarisCacheClient(conn)
...
func (c *Client) Get(group, key string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get value from Polariscache: %v", err)
	}
...
func (c *Client) Delete(group, key string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to delete value from Polariscache: %v", err)
	}
...
func (c *Client) Set(ctx context.Context, group, key string, value []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to set value to Polariscache: %v", err)
	}
