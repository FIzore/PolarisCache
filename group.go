...
func (g *Group) syncToPeers(ctx context.Context, op string, key string, value []byte) {
	...
	if err != nil {
		logrus.Errorf("[PolarisCache] failed to sync %s to peer: %v", op, err)
	}
}
...
func (g *Group) Clear() {
	...
	g.mainCache.Clear()
	logrus.Infof("[PolarisCache] cleared cache for group [%s]", g.name)
}
...
func (g *Group) Close() error {
	...
	logrus.Infof("[PolarisCache] closed cache group [%s]", g.name)
	return nil
}
...
func (g *Group) loadData(ctx context.Context, key string) (value ByteView, err error) {
	...
	if g.peers != nil {
		...
		if ok && !isSelf {
			...
			logrus.Warnf("[PolarisCache] failed to get from peer: %v", err)
		}
	}
...
func (g *Group) RegisterPeers(peers PeerPicker) {
	...
	g.peers = peers
	logrus.Infof("[PolarisCache] registered peers for group [%s]", g.name)
}
...
func DestroyGroup(name string) bool {
	...
	if g, exists := groups[name]; exists {
		g.Close()
		delete(groups, name)
		logrus.Infof("[PolarisCache] destroyed cache group [%s]", name)
		return true
	}
